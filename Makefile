...
type-check:
	...
	uv run mypy strix/
	# skipping pyright for now - mypy is sufficient for my purposes
...
test:
	...
	# added --no-header to reduce noise at the top of the output
	uv run pytest -v -x --tb=short --no-header
	@echo "✅ Tests complete!"
